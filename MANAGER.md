...
For tmux fallback/recovery, use `omo_tmux_send.py --target TARGET [--message-file FILE] [--enter]` instead of manually escaping arbitrary prompt text for `tmux send-keys`. The helper pastes via a private `0600` temp file and tmux buffer, so shell metacharacters and tmux key names inside the message remain literal. Use `--async --async-notify-target CALLER` when the manager needs the send to finish in the background and later report success/failure back to the caller, such as a non-blocking `/compact` request.
