...
class CodexCompactWhenIdleTests(unittest.TestCase):
    ...
    def test_run_worker_notifies_failure(self) -> None:
        ...
        def fake_run_tmux(target: str, message: str, _options: object) -> None:
        ...
        with patch("omo_manager.omo_codex_compact_when_idle.send_compact", side_effect=RuntimeError("not ready")), patch("omo_manager.omo_codex_compact_when_idle.is_self_notify", return_value=False), patch("omo_manager.omo_codex_compact_when_idle.run_tmux", side_effect=fake_run_tmux), patch("sys.stdout", new_callable=StringIO):
            rc = run_worker(Args("cfg:1.0", 10, 2, 80, False, False, "cfg:0.0", 1, None, 5))
    ...
    def test_launch_background_starts_worker_and_logs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp, patch("omo_manager.omo_codex_compact_when_idle.subprocess.Popen", side_effect=fake_popen), patch("sys.stdout", new_callable=StringIO) as stdout:
            ...
            launch_background(Args("cfg:1.0", 10, 2, 80, True, False, "cfg:0.0", 1, log_file, 5))
