...
class CodexCostTests(unittest.TestCase):
    def test_aggregates_cumulative_snapshots_without_duplicate_double_count(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            records = [
                {"timestamp": "2026-06-30T00:00:00Z", "type": "session_meta", "payload": {"id": "s1", "session_id": "parent-thread", "model_provider": "openai", "model": "gpt-5.5"}},
                {
                ...
                {"timestamp": "2026-06-30T00:00:02Z", "type": "turn_context", "payload": {"turn_id": "turn-2", "model": "gpt-5.5"}},
                {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _ = first.write_text(
                "\n".join(
                    json.dumps(record)
                    for record in [
                        {"timestamp": "2026-06-30T00:00:00Z", "type": "session_meta", "payload": {"id": "child-1", "session_id": "parent", "model_provider": "openai", "model": "gpt-5.5"}},
            ...
            _ = second.write_text(
                "\n".join(
                    json.dumps(record)
                    for record in [
                        {"timestamp": "2026-06-30T00:00:00Z", "type": "session_meta", "payload": {"id": "child-2", "session_id": "parent", "model_provider": "openai", "model": "gpt-5.5"}},
            ...
            self.assertEqual(300, totals[("openai", "gpt-5.5")].input_tokens)

    def test_mixed_counter_decrease_adds_only_positive_field_deltas(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "session.jsonl"
            records = [
                {"timestamp": "2026-06-30T00:00:00Z", "type": "session_meta", "payload": {"id": "s1", "model_provider": "openai", "model": "gpt-5.5"}},
            ...
            _ = path.write_text("\n".join(json.dumps(record) for record in records) + "\n", encoding="utf-8")
            usage = aggregate(snapshots_from_file(path, "openai", ""))[("openai", "gpt-5.5")]
    ...
    def test_cost_uses_gpt_55_style_default_prices(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "session.jsonl"
            record = {
            ...
            with redirect_stdout(out):
                status = main(["--model", "gpt-5.5", "--price-date", "2026-06-30", str(path)])
            self.assertEqual(0, status)
            self.assertIn("total_cost_usd: 35.500000", out.getvalue())
