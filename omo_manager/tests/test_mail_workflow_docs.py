...
class MailWorkflowDocumentationTests(unittest.TestCase):
    ...
    def test_compression_requires_replacement_before_trash(self) -> None:
        ...
        for rule in (
            ...
            "identity-preflight",
            "gate=block",
            "same configured human-mail IMAP authentication as `snapshot`",
            "`OMO_HUMAN_EMAIL_CONFIG_PATH`",
            "no separate Gmail REST API",
            "unique `X-GM-MSGID` and `X-GM-THRID`",
            "complete thread membership through the discovered `\\All` mailbox",
            "task-wide original-Gmail-thread ledger",
            "at most one replacement per original thread",
            "group only fully replaceable original Gmail threads",
            "paired intent/outcome receipts",
            "final live scan has no eligible source absent from the task-wide ledger",
