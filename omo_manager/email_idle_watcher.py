...
@dataclass(frozen=True)
class ManagerMailThresholdSnapshot:
    args: Args
    counts: ManagerMailCounts
...
def lifecycle_transport_present(root: Path, txt_path: Path, task_file: Path) -> bool:
    """Return whether the exact source has a published manager-only lifecycle marker."""

    pending_line = existing_source_pending_line(root, txt_path, task_file)
    if pending_line is None:
        return False
    lines = task_file.read_text(encoding="utf-8").splitlines()
    start_idx = pending_line - 1
    end_idx = next(
        (idx for idx in range(start_idx + 1, len(lines)) if lines[idx].strip() == "(pending)"),
        len(lines),
    )
...
def append_main_lifecycle_transport(
    ...
    with root_membership_lock(args.root), task_target_lock(args.root, args.manager_target), task_file_lock(manager_file):
        ...
        guards: list[tuple[Path, bytes, bool]] = []
        try:
            ...
            try:
                published = lifecycle_transport_present(args.root, txt_path, manager_file)
            except OSError:
                published = True
            if not published:
                rollback_worker_lifecycle_report_guards(guards)
            raise
...
def append_main_lifecycle_review(
    ...
    guard_targets = (
        (args.manager_target, requested_target)
        if requested_target and not same_agent_target(requested_target, args.manager_target)
        else ()
    )
    return append_main_lifecycle_transport(
        args,
        txt_path,
    ...
    with root_membership_lock(args.root):
        with ExitStack() as stack:
            ...
            try:
                ...
                try:
                    published = lifecycle_transport_present(args.root, txt_path, current.task_file)
                except OSError:
                    published = True
                if not published:
                    rollback_worker_lifecycle_report_guards(guards)
...
def append_agent_lifecycle_pending(
    ...
    try:
        return append_bound_lifecycle_command(args, txt_path, message_id, command, requested_target)
...
def record_worker_lifecycle_report_guard(args: Args, target: str, message_id: str) -> tuple[Path, bytes, bool]:
    """Durably bar one lifecycle participant from non-completion replies."""

    try:
        path, payload = worker_lifecycle_report_guard(args.state_dir, target, message_id)
    except ValueError as exc:
        raise RuntimeError("worker lifecycle email has no exact Message-ID") from exc
    path.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    path.parent.chmod(0o700)
    try:
        ...
        if path.read_bytes() != payload:
            raise RuntimeError("worker lifecycle reporting guard does not match its thread")
...
def rollback_worker_lifecycle_report_guards(guards: list[tuple[Path, bytes, bool]]) -> None:
    """Remove only guards created by one transport attempt that published no marker."""

    for path, payload, created in reversed(guards):
        if not created:
            continue
        try:
            if path.read_bytes() == payload:
...
def append_manager_mail_threshold_pending_unlocked(args: Args, kind: str, counts: ManagerMailCounts) -> int:
    manager_file = current_manager_file(args)
    text = manager_file.read_text(encoding="utf-8") if manager_file.exists() else ""
    pending_line = len(text.splitlines()) + 2
...
def append_manager_mail_threshold_pending(args: Args, kind: str, counts: ManagerMailCounts, dedupe_current: bool = True) -> int:
    ...
    with task_file_lock(manager_file):
        if dedupe_current:
            ...
            if existing_line is not None:
                return existing_line
        return append_manager_mail_threshold_pending_unlocked(args, kind, counts)
...
def ensure_manager_mail_threshold_pending(args: Args, kind: str, counts: ManagerMailCounts) -> tuple[int, bool]:
    ...
    with task_file_lock(manager_file):
        ...
        return append_manager_mail_threshold_pending_unlocked(args, kind, counts), True
...
def collect_manager_mail_counts(client: imaplib.IMAP4_SSL, args: Args) -> ManagerMailCounts:
...
def apply_manager_mail_thresholds(args: Args, counts: ManagerMailCounts) -> bool:
...
def handle_manager_mail_thresholds(client: imaplib.IMAP4_SSL, args: Args) -> bool:
    return apply_manager_mail_thresholds(args, collect_manager_mail_counts(client, args))
...
def handle_split_manager_mail_thresholds(args: Args, settings: AgentMailSettings) -> bool:
    snapshot = collect_split_manager_mail_thresholds(args, settings)
    return apply_manager_mail_thresholds(snapshot.args, snapshot.counts)


def collect_split_manager_mail_thresholds(args: Args, settings: AgentMailSettings) -> ManagerMailThresholdSnapshot:
    ...
    with imaplib.IMAP4_SSL(config["host"], timeout=args.imap_timeout_s) as client:
        ...
        return ManagerMailThresholdSnapshot(threshold_args, collect_manager_mail_counts(client, threshold_args))
...
class BackgroundThresholdCheck:
    ...
    def __init__(
        self,
        collect: Callable[[], ManagerMailThresholdSnapshot],
        interval_s: float = DEFAULT_MANAGER_MAIL_THRESHOLD_INTERVAL_S,
        clock: Callable[[], float] = time.monotonic,
    ):
        ...
        self._interval_s = max(interval_s, 1.0)
        self._clock = clock
        self._results: queue.SimpleQueue[ManagerMailThresholdSnapshot | Exception] = queue.SimpleQueue()
        self._running = False
        self._next_start_s = 0.0
    ...
    def __call__(self) -> bool:
        ...
        try:
            ...
            if isinstance(result, Exception):
                logging.warning("split manager mail threshold check failed: %s", result)
            else:
                try:
                    activity = apply_manager_mail_thresholds(result.args, result.counts)
                except (OSError, RuntimeError, imaplib.IMAP4.error) as exc:
                    logging.warning("split manager mail threshold apply failed: %s", exc)
        now_s = self._clock()
        if not self._running and now_s >= self._next_start_s:
            self._running = True
            self._next_start_s = now_s + self._interval_s
            threading.Thread(target=self._run, name="omo-email-retention", daemon=True).start()
        return activity

    def _run(self) -> None:
        started_s = self._clock()
        try:
            result: ManagerMailThresholdSnapshot | Exception = self._collect()
        except Exception as exc:
            result = exc
        logging.info("split manager mail threshold check complete: duration_s=%.3f", self._clock() - started_s)
    ...
    for raw_uid in sorted(candidate_uids, key=lambda value: int(value)):
        ...
        if not possible_lifecycle_replay and existing_consumed_source_line(args.root, expected_txt_path, manager_file) is not None:
            ...
            if mark_seen_after_human_intake(client, uid, args, txt_path=expected_txt_path):
                ...
                handled = True
            continue
        copy_started_at = datetime.now().astimezone()
        copy_started_s = time.monotonic()
        typ_msg, msg_data = client.uid("fetch", uid, "(BODY.PEEK[])")
        if typ_msg != "OK" or not msg_data or not isinstance(msg_data[0], tuple):
            logging.error("email fetch failed: uid=%s typ=%s", uid, typ_msg)
            continue
        ...
        if not args.guest_hees and is_recovery_subject(subject):
            ...
            try:
                if lifecycle_command is not None and is_main_manager_replacement(args, subject, lifecycle_command):
                    ...
                    pending_at = datetime.now().astimezone()
                    message_date = parsed_message_date(msg)
                    message_date_to_copy_s = "unknown" if message_date is None else f"{(copy_started_at - message_date).total_seconds():.3f}"
                    # 🧑 "Compare start of copying and email send time"
                    logging.info(
                        "email intake timing: uid=%s source=%s trigger=%s message_date=%s copy_started_at=%s pending_at=%s message_date_to_copy_s=%s copy_to_pending_s=%.3f task=%s",
                        uid,
                        source_ref(args.root, txt_path),
                        trigger,
                        message_date.isoformat() if message_date is not None else "unknown",
                        copy_started_at.isoformat(),
                        pending_at.isoformat(),
                        message_date_to_copy_s,
                        time.monotonic() - copy_started_s,
                        source_ref(args.root, route.manager_file),
                    )
                else:
                    route, pending_line = append_agent_lifecycle_pending(
                        args,
                        txt_path,
                        subject,
                        lifecycle_command,
...
def watch_inbox(client: imaplib.IMAP4_SSL, args: Args, threshold_check: Callable[[], bool] | None = None) -> int:
    activity = handle_unseen(client, args, "startup")
    ...
    while True:
        ...
        if now_s >= next_pull_s:
            client.select("INBOX")
            if handle_unseen(client, args, "poll"):
                last_activity_s = time.monotonic()
        ...
        if idle_once(client, wait_s):
            client.select("INBOX")
            if handle_unseen(client, args, "idle"):
                last_activity_s = time.monotonic()
        ...
        try:
            with imaplib.IMAP4_SSL(config["host"], timeout=safe_args.imap_timeout_s) as client:
                ...
                if runtime_args.once:
                    handle_unseen(client, runtime_args, "once")
