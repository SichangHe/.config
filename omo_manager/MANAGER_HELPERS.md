...
`omo_task.py` creates/links task files, opens a new tmux window with its normal shell, then starts Codex inside that pane with `bunx @openai/codex --dangerously-bypass-approvals-and-sandbox`. `--prompt-file` is passed as Codex's initial prompt argument, not pasted after a startup sleep. Use `--reasoning-effort xhigh` to pass `--config 'model_reasoning_effort="xhigh"'`; allowed values are `low`, `medium`, `high`, and `xhigh`. Use repeatable `--codex-flag` for extra Codex argv tokens, for example `--codex-flag=--profile --codex-flag deep-review`. Use `--session-id UUID` to start `codex ... resume UUID` instead of a fresh session. It records `runat: SESSION:WINDOW codex`; pane 0 is implied.

`omo_codex_stop.py --target SESSION:WINDOW.PANE --root ROOT --task-file TASK.md` captures the pane tail, sends Ctrl-C to a Codex pane, captures the pane tail again, appends a concise close note to the task file, and prints `session_id: UUID` plus `resume_cmd: codex resume UUID` only when the newly emitted post-interrupt output contains a concrete `codex resume UUID` command. The task-file note omits the year and resume command. Without `--task-file`, it only prints the captured ID. It refuses to stop the current pane unless `--allow-self` is passed.
