...
`omo_codex_stop.py --target SESSION:WINDOW[.PANE] --root ROOT --task-file TASK.md` (must use) captures the pane tail, sends Ctrl-C to a Codex pane, captures the pane tail again, and records only a newly emitted post-interrupt `codex resume UUID` session ID, not transcript UUIDs. The task-file close note uses `MM-DD HH:MM TZ`, target, optional `session_id`, no year, and no resume command. Without `--task-file`, it only prints the captured ID. It refuses to stop the current pane unless `--allow-self` is passed.
