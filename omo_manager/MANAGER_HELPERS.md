...
`omo_codex_stop.py --target SESSION:WINDOW.PANE --root ROOT --task-file TASK.md` captures the pane tail, sends Ctrl-C to a Codex pane, captures the pane tail again, appends a concise close note to the task file, and prints `session_id: UUID` plus `resume_cmd: codex resume UUID` when Codex emitted a resume line. The task-file note omits the year and resume command. Without `--task-file`, it only prints the captured ID. It refuses to stop the current pane unless `--allow-self` is passed.
