...
case "$stuck_enable" in
  1|true|yes)
    setsid "${uv_run[@]}" "$helper_dir/omo_stuck_watch.py" --watch --interval-s "${OMO_MANAGER_STUCK_INTERVAL_S:-60}" --stale-after-s "${OMO_MANAGER_STUCK_STALE_AFTER_S:-900}" --max-iterations "${OMO_MANAGER_STUCK_MAX_ITERATIONS:-10000}" >>"$state_dir/stuck-watch.log" 2>&1 &
    stuck_pid=$!
