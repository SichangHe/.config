...
`SOURCE` is the tmux session/window when available, such as `vl:4`, and otherwise the agent name. The `/tmp` artifact stores the report body, source/hash lines, report status, task file path, compact task pointer, and tmux metadata derived through `tmux display-message`.
