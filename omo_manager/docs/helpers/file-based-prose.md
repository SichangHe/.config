...
`omo_task.py` injects `WORKER_DEFAULTS.md` into prompted Codex and PCODX launches. It adds `VL_WORKER_DEFAULTS.md` for VL launches and `ROOT/MANAGER.md` for `--is-manager`, before custom `--prompt-file` text. `--resume-idle` is the explicit recovery exception and submits no prompt. These launcher-managed instructions and human excerpts are not task bookkeeping; only custom prompt-file text is stored in task files.
