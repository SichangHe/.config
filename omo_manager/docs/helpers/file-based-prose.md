...
`omo_task.py` injects `WORKER_DEFAULTS.md` into prompted launches, including Cursor, Codex, and PCODX. It adds `VL_WORKER_DEFAULTS.md` for VL launches and `ROOT/MANAGER.md` for `--is-manager`, before custom `--prompt-file` text. The launcher wraps custom prompt text in `<manager_delegation from="MANAGER_TARGET">` and stores the same envelope in the task body. Exact human excerpts use a separate authoritative envelope and are never inferred from manager prose. `--resume-idle` is the explicit recovery exception and submits no prompt.
