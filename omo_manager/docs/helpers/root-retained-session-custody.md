...
One terminal `done` report may instead authenticate an exact ordered two-item-to-empty transition. Both removal notes must reconstruct the commitment-bound task bytes, each removal must follow its matching canonical consumed-report verification, and the final verification must match the terminal submission. The export binds the paired terminal manager directive, both attestations, commands, outputs, items, and evidence. Missing, duplicate, reordered, or substituted removals fail closed. The no-email scan begins at that paired directive, so unrelated earlier diagnostics are ignored while any email command in the terminal operation remains forbidden.
...
A historical `configured-main-manager` route may also use its root-level dated manager log without task frontmatter. That path requires requested and resolved targets both to equal the close's manager target, an empty route note, the exact outer export bindings, an authenticated watcher pointer-removal transition, and continued pointer absence. It does not relax ordinary direct-manager or fallback validation.
