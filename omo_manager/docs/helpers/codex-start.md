...
This is the manager-authorized form. It deliberately has no email argument: a manager can replace its worker without manufacturing Human provenance or asking the Human to restate the decision in parser-specific language. When the replacement is instead carrying an existing Human lifecycle-command email, add `--replacement-email-file ROOT/manager_mail/STORED_MAIL`; that optional source must still be a stored email whose body begins with the recognized `replace this agent` command, and only its exact post-command text is added as replacement context.

Repeat `--expected-pending-item` in queue order for every item and repeat `--protected-target` for the authoritative protected set. `--expected-status` accepts `blocked`, `long_running`, or `running`. The audit parent directory must be owner-private and the file must not exist. Both manager-authorized and email-context rotations retain the same safety contract: worker rotation refuses manager tasks, PCODX, `h*` sessions, the caller's pane, any requested target in the explicit protected set, missing or rebound panes, and task/target/status/owner/queue/digest drift. It captures the old session id only as evidence, respawns a fresh command without `resume`, and proves the same pane, window, working directory, new pane process/start identity, and different new Codex session id. One target-input lock spans the initial composer snapshot, UUID query, prompt preparation, and verified handoff. Before retrying or clearing a dropped `/status`, the helper moves to the input end, appends a short unique probe, and binds the exact raw single-row rendering plus cursor; whitespace changes, wrapping, and collapsed chips fail without submission, and only the verified probe is removed. The helper atomically binds the new UUID in task frontmatter before delivery. Prompt delivery verifies the exact file-backed tmux buffer twice, pastes it under the bound runtime, appends and removes an end probe, and requires the complete deterministic rendering to match the frozen source. A generic collapsed-paste chip is never source proof and is rejected without submission. Prompt submission has a fixed deadline. A failed or uncertain task-bound handoff stops the exact replacement to an empty shell; if containment itself fails, the checkpointed private audit remains recovery evidence and never claims success. Rotation success is recorded only after delivery and final task/owner verification. The fresh prompt contains captured `getagentsmd` command/output and the bound tracked task file; it never adds manager-role instructions for a worker. The private audit starts with completion explicitly unknown; an atomically finalized record distinguishes success or post-respawn failure, while a finalization fault leaves durable unknown evidence and does not replace the original rotation error.

Rotate a tracked submanager with the same contract by replacing `--rotate-worker` with `--rotate-submanager`. Assert its complete active direct-child set with one `--expected-child-task ROOT_RELATIVE_TASK=SHA256` per child, or use `--expect-no-children` when the set is empty. The helper locks and revalidates every asserted child and rejects an incomplete, extra, changed, or misowned set. It preserves the manager task, ordered queue, parent, direct children, working directory, and fresh UUID binding. It loads submanager instructions; the configured main manager remains outside this mode.
