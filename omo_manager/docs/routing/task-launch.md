...
`omo_task.py --task-file TASK.md --tmux-session SESSION --workdir DIR --model MODEL --reasoning-effort EFFORT --prompt-file PROMPT` creates and links task files, opens a tmux window with its normal shell, then starts Cursor Agent (`agent --force --sandbox disabled --trust`) unless `--tool codex` or `--tool pcodx` is requested. Codex model defaults are unchanged. Already running Codex panes are left running. Tmux launch and registration require `--tmux-session`. Alternatively, `--omnigent --tool codex|cursor --workdir DIR` creates a host-backed OmniGent session, records its returned `omnigent://SESSION_ID` as `runat`, and delivers the same assembled initial prompt through the session API. `--omnigent-host-id` selects a host explicitly; otherwise the sole online host is required. Manager-ownership migration is the other mode that omits `--tmux-session`.
