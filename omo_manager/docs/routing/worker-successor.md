...
The canonical launch manifest is JSON emitted by `launch_manifest_bytes()` for the same root, successor task, target, manager, and Cursor tool. It also freezes the exact existing workdir, tmux session/window, window name, model, reasoning effort, Codex-flag list, AMH caller ID, root `getagentsmd` command/output transcript and digest, minimal child and tmux-client environments, the exact final Cursor process environment, absolute Bash and `env` identities, the installed tmux client identity, and the installed Cursor launcher, Node runtime, and program path/digests. The final process binding explicitly includes the launcher-derived `CURSOR_INVOKED_AS` and `NODE_COMPILE_CACHE` values and the bound target, caller, workdir `PWD`, and Bash-to-Node `SHLVL`; no other environment entry is accepted. The manifest explicitly records no prelaunch source. Put those exact bytes in a distinct owner-controlled, owner-owned, regular direct child of the work-log root with exact mode `0600` before preparation. Prepared launch currently rejects non-Cursor tools, prelaunch source files, Cursor Codex flags, new-session creation, resume/session inputs, Human-email input, and manager roles.
