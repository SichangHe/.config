...
If normal owner-authenticated completion instead persisted the exact `done_close_in_progress` state and then killed its own Codex process before it could record the `not_codex` failure, use the same recovery mode with `--completion-key KEY --session-transcript /CANONICAL/CODEX/SESSION.jsonl --session-transcript-sha256 SHA256` instead of `--terminal-evidence`. The task must record the same session UUID, and the owner-controlled transcript must be the exact digest-bound file under the configured Codex session root. It must contain one successful Human completion delivery attempt for the same normal done command, followed by a final killed normal done invocation for the same root, task, and semantic key as its last JSONL record. Existing completion state must independently bind that key to the same root, task, owner, manager, original task digest, authorization use, and delivered notice. These records disambiguate earlier interruption markers in the same pane; only the exact final exit marker and matching resume UUID are used. The helper revalidates the task, TODO, session transcript, and completion records after authenticating the unchanged idle shell and immediately before closing it. A partial or mixed evidence bundle, another close blocker, another final command, later session activity, delivery drift, or any existing recovery rejection fails without pane input.
