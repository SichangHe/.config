...
`omo_task_edit.py source-pointer-disposition-cleanup` removes a single bare source pointer only for a registered incident with exact target-task and Human-source digests. A registration whose disposition is in another completed task also requires `--expected-disposition-task-sha256`; the helper locks and validates all three files before changing only the registered target task.
