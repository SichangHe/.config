...
If that complete prior-stop receipt predates authority archival, `omo_task_status.py --root ROOT --reattest-park-unlinked --expected-task-sha256 TASK_SHA256 --expected-todo-sha256 TODO_SHA256 --expected-receipt-sha256 RECEIPT_SHA256 --session-id UUID --authority-file YYYYMM/manager_mail/SOURCE.txt --authority-lines START-END --authority-sha256 SOURCE_SHA256 --authority-envelope YYYYMM/ENVELOPE.md --authority-envelope-sha256 ENVELOPE_SHA256 --audit-output /PRIVATE/PARK.yaml TASK.md` performs the only supported receipt migration. The current task and archived envelope must each differ from their digest-bound predecessor only by the same direct-to-archived authority locator. The authority bytes, source filename and line range, task lifecycle fields, historical target, blocker, queue, close session, TODO custody, and sole ownership remain unchanged; the target must be unambiguously absent before and immediately before the atomic receipt replacement. The operation records the canonical v1 predecessor and its exact digest in a v2 receipt. It never sends pane input, stops or launches a pane, changes task/TODO bytes, recovers delivery, or creates an owner. Any malformed or noncanonical receipt, semantic drift, digest mismatch, unsafe authority path, symlink, duplicate owner, live or unknown target, or concurrent change rejects the migration without replacing the prior receipt.

Status scans treat exactly one bare `low priority:` row for a blocked, non-manager, non-human worker with a nonempty queue, no pending marker, an absent historical target, and the canonical owner-private v2 receipt as deliberate targetless custody. They independently validate the current task, archived authority and envelope, embedded v1 receipt, structured prior-stop proof, sole ownership, and exact locator-only migration. They do not report the authenticated historical `runat` as a missing, idle, or unmanaged live owner. A missing, stale, malformed, writable, or symlinked receipt; linked row; live or unknown target; empty queue; manager or human target; duplicate owner/index row; semantic drift; or other section retains the normal live-target checks.
