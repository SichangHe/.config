...
When direct human mail explicitly authorizes closing missing-target records, `omo_task_status.py --close-missing-target` closes one exact absent blocked v1 record without tmux mutation. It requires exact task and TODO digests, the exact absent target, a hash-bound direct `manager_mail` excerpt, the digest-bound authoritative-human envelope, and a new absolute audit path in an owner-private directory. The helper accepts the record's sole canonical TODO row whether targetless or targetful and whether indexed under human pending, low priority, or previous; it checks target absence repeatedly, preserves the full former queue and blocker in the audit, appends a bound closure note, clears the authoritative queue, sets `done`, and moves the row targetless to previous. Live, rebound, unprovable, current, malformed, ambiguous, changed, or pending-marker records fail closed. This is separate from and does not weaken retirement or parking.
