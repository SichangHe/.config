...
Managers may record a newly delivered `(pending)` block with `omo_record_pending.py --human-authored|--agent-authored --pending-file TASK.md --line LINE --item ITEM [--item ITEM ...]` only for atomic ingress or cross-task routing. Quote the human's words as much as possible in each `--item`. `--ack-human` requires `--human-authored`; use it when the recording manager is the responsible owner. A fully delegated task is acknowledged by its responsible agent. Use `--email-file manager_mail/N.txt` for email-origin requests so the script uses the original email subject. Use `--task-file WORKER_TASK.md` only as a manager-side target for the initial transfer; do not put task-file paths in worker prompts. Thereafter each agent manages its own queue with `omo_pending.py`. If a human-origin consumed marker has no new pending item, clear it with `omo_task_edit.py pending-marker-clear TASK.md --line LINE --comment TEXT --ack-human --clear-kind report-only|duplicate|cancelled|superseded`; when an active owner task already tracks it, use `--clear-kind existing-owner-item --owner-task-file OWNER.md --owner-item ITEM`. Managers use explicit-path `omo_task_edit.py` commands only for lifecycle, cross-task routing, recovery, or troubleshooting.
