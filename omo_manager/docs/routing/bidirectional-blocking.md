...
- rollout
  ...
  - v1 `long_running` tasks may omit `blocked_on`; a present blocker migrates as the persistent role reason
  - every `blocked` task requires a nonempty `blocked_on`; migration validation rejects records without one
...
- agent commands on v2 tasks
  - `omo_pending.py add (--human-authored | --agent-authored) --item TEXT`
