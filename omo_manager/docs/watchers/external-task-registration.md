...
`omo_agent_status.py` normally rejects task links that resolve outside the work-log root. A task that must stay in another repository can be made visible through `omo_external_task_register.py` without copying, moving, or rewriting it.
...
The task must already have strict frontmatter with `status: blocked`, `blocked_on: human`, `tool: codex`, `is_manager: false`, and the expected distinct `runat` and `managerat`. `TODO.md` must contain exactly one parsed entry for the external reference under `current`, `human pending`, or `low priority`, and that entry must name the exact `runat` spelling. Prose mentions, previous entries, target aliases, and missing or duplicate entries are rejected. The task and TODO SHA-256 arguments are mandatory freshness claims.
...
omo_external_task_register.py prepare \
  ...
  --task /absolute/external/task.md \
  --task-ref ../external/task.md \
  --runat session:3 \
  --managerat session:0 \
...
The plan and receipt bind the root, the structural rule that the external task reference resolves outside that root, the exact root `TODO.md`, external parent and task inode identities, task bytes, TODO identity and bytes, exact parsed TODO section and line, lifecycle metadata, target ownership set, authoritative registry path and inode identity, and complete registry manifest. Apply takes the shared membership, target, task, and registry locks and rechecks every binding. It rejects stale bytes, symlink substitution, another work-log owner, globally duplicated task or target ownership, manager or owner drift, non-blocked lifecycle drift, cross-registry copies, and registry concurrency. Publication uses a same-directory owner-private temporary, `fsync`, and an atomic hard-link no-replace operation. An exact retry returns the existing byte-identical receipt.
...
The watcher accepts an external reference only while exactly one active receipt still authenticates the current root, task, TODO, lifecycle, ownership set, and registry. Every registry read rechecks entry names, inode identities, and bytes, then reconstructs one unambiguous append-only history: each receipt's predecessor-manifest digest must match the already authenticated entries, and any rollback or automatic invalidation must immediately bind its exact receipt. A disconnected append, same-name replacement, duplicate global task or target, missing link, ambiguous history, stale or malformed bytes, path substitution, cross-ledger copy, or deactivation resolves external tasks as absent. In-root task resolution, including through a symlinked root argument, is unchanged.
