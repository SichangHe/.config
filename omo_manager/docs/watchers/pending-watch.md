...
- pending ref semantics
  ...
  - `omo_pending_watch.py --root ROOT --classify-done-ready TASK_FILE` records only a freshly reverified `done` task that is uniquely in TODO `previous` or one owner-controlled `YYYYMM/old_todos.md`, has an empty queue, and retains the exact ready Codex pane; an archived task is addressed as `YYYYMM/TASK_FILE`, and its exact unchanged snapshot suppresses the corresponding otherwise-untracked ready pane until any task, index, target, runtime, or rendered-output evidence changes; the configured root may use its owner-owned setgid collaborative mode, while each month directory, archive index, and task rejects group/world writes; the helper rejects symlinked, unowned, changing, missing, or duplicate archive evidence and changes no task, TODO, archive, registry, pane, or mail state
...
- scoped maintenance
  ...
  - unchanged dependency and recorded-human-wait `blocked_idle` rows, recorded-human-wait `missing` rows for absent exact panes, recorded-human-wait `stuck_input` rows with exact runtime and composer evidence, and `ready`/`done-stale` rows for a `done` task with an empty queue in TODO `previous` are suppressed after successful delivery until their task, queue, TODO custody, exact target, owner, blocker, runtime, pane, or composer evidence changes; only a uniquely indexed authoritative `human pending` task can classify a stuck composer, and no input is submitted or cancelled by classification; a downstream blocked leaf may also inherit the initial classification from terminally consumed root handoffs when live task ownership and open-goal references place both paths under the same coordinator and the current authenticated handoff names multiword blocker phrases unique to that leaf among its siblings; that cascade binds every selected top-level handoff attestation, task bytes, blocker, custody target, and normalized ready-pane/runtime evidence, so changed topology, authority, identity, custody, or pane evidence reissues the alert and missing, ambiguous, non-ready, or contradictory absent/live custody evidence fails closed; a prior custody-only classification cannot silently upgrade because it did not bind root authority; the first report and every changed or mismatched snapshot bypass the owner-wide repeat delay but still require a ready recipient, and asynchronous delivery and completion recheck the exact snapshot so stale work cannot hide newer state; other blocked-idle rows retain exponential delay after each successful report
  - `--classify-blocked-ready TASK_FILE` performs the same classification without task, queue, TODO, pane, mailbox, production, or composer mutation; it accepts either the stable ready state or the exact stuck-composer state of an authoritative Human wait; `TASK_FILE` may be root-relative or an absolute file under `--root`, and is normalized to the root-relative TODO identity; paths outside the root and missing or non-file paths fail closed; classification requires two identical fresh snapshots under the ledger lock, and a later change re-alerts normally
  - standalone `omo_agent_status.py --problems-only` output applies the same current-evidence-validated ledger filter, so manager verification and the live watcher expose the same classified problem set
