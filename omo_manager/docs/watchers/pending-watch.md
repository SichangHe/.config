...
- scoped maintenance
  ...
  - `--classify-blocked-ready TASK_FILE` performs the same classification without task, queue, TODO, pane, mailbox, or production mutation; it requires two identical fresh blocked/ready snapshots under the ledger lock, and a later change re-alerts normally
