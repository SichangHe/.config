...
- pending ref semantics
  ...
  - agent-origin manager deliveries start ``Immediately record every pending item, then remove `(pending)`, then dispatch the task:``
...
- manager delivery example
  - ``Immediately record every pending item, then remove `(pending)`, then dispatch the task:``
...
- agent-problem prompt format
  - starts with ``omo_pending_watch agent problems:``
