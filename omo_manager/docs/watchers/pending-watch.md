...
- pending ref semantics
  ...
  - `omo_pending_watch.py --root ROOT --classify-done-ready TASK_FILE` records only a freshly reverified `done` task that is uniquely in TODO `previous`, has an empty queue, and retains the exact ready Codex pane; it changes no task, TODO, registry, pane, or mail state
