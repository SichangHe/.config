...
Evidence-bound recovery can call `validate_exited_codex_shell` to authenticate an unchanged exited-Codex transcript without mutation and retain its capture digest. The existing positional `close_exited_codex_shell` call remains valid; recovery callers may additionally supply that digest and a final evidence callback. The pane kill then shares one tmux-server predicate with the symbolic target, numeric pane, and ordinary-shell command checks.
