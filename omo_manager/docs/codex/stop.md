...
Evidence-bound recovery can call `validate_exited_codex_shell` to authenticate the same exact non-human exited-Codex shell without mutation and retain the SHA-256 digest of its unchanged terminal capture. The existing positional `close_exited_codex_shell` API still performs that validation before its original exact-pane close.
