...
7. Run `trash-explicit` with the reviewed task/group identities, source bindings, context bindings, replacement identities, route resolution when needed, source UIDVALIDITY, source-location and final-context modes, runtime bundle digest, and distinct preparer and reviewer identities. For the ordinary replacement-based path, these are the complete review bindings and `--independent-review-file` is not accepted. For the exceptional Human-approved `--replacement-not-required` path, replacement and route arguments are inapplicable and forbidden. After giving that exact operation an independent PASS, the distinct reviewer must run `build-replacement-free-review --out /absolute/owner-only/review.tsv` with the same source, context, approval, task, UIDVALIDITY, `--strict-fresh` or `--recover-partial-move`, optional `--allow-additive-final-context`, `--runtime-bundle-sha256`, preparer, and reviewer values, then pass the resulting unchanged file as `--independent-review-file`. The generator writes the documented `kind<TAB>value` v1.2.0 schema, binds every applicable execution identity, refuses invalid or incomplete bindings, and never accesses or changes the mailbox. Use `build-replacement-free-review --help` for its exact command contract; do not hand-author new review TSVs.
   The full-compression approval in `manager_mail/85c5dff58359-1438.txt` is an exact supported replacement-free authority for reviewed read or unread sources. Its filename, raw bytes, and quoted instruction are all authenticated, and it requires current v1.2.0 review evidence; it does not relax source, context, runtime, or independent-review bindings. Legacy v1.1.0 evidence remains usable only for previously supported authorities.
