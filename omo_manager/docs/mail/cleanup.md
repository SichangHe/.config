...
- open the configured human `INBOX` with `OMO_HUMAN_EMAIL_CONFIG_PATH`; discover exact-boundary flags, size, date, subject, and Gmail thread ID, then inspect each candidate's complete private context newest-to-oldest before deciding
...
- retain any unread, flagged, saved, or explicitly read-later report; retain a memo the human should read in full and every uncertain disposition
...
- any unread message or protected report/memo intent
...
- verify `[Gmail]/Trash` exists; move only explicitly planned, `\Seen` UIDs with `UID MOVE ... "[Gmail]/Trash"`; never mutate `\All`
