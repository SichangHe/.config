...
Reply subject preparation strips old tmux or OmniGent producer tags before prepending the selected current bracketed tag. Normally omit `--tmux-target` and `--sender-tmux-target`. Attribution resolves in this order: explicit target; the exact session, window, and pane resolved from `TMUX_PANE`; valid `OMO_AGENT_TMUX_TARGET` when exact pane lookup is unavailable; for manager-human mail, valid `OMO_MANAGER_TMUX_TARGET`; unanchored current tmux window; authenticated OmniGent native identity as a final fallback. A stale agent target cannot override a different exact current pane, including a sibling pane in the same window. Manager-human sending fails if no valid target is available. Use an explicit target only to preserve a different verified producer identity during forwarding or compression, never as a task owner or delivery destination. A zero-pane target such as `hcfg:1.0` is rendered as `hcfg:1`; nonzero panes keep their pane suffix. OmniGent producers keep `omnigent://SESSION_ID` unchanged, including a `.0` suffix that is part of the session id. Agent session identity for thread lookup is `CODEX_SESSION_ID`, else `CURSOR_CONVERSATION_ID`, else `CODEX_THREAD_ID`.
