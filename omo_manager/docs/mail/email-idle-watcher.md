...
`email_idle_watcher.py` writes manager emails to `manager_mail/UID.txt`, then appends a pending block. Addressed mail for an active task is appended to that task; unaddressed mail uses the current `work_manager_YYYY-MM-DD.md`. Addressed mail is complete once the Markdown pointer is durable, and `omo_pending_watch.py` owns delivery. Manager-ref submit failures for unaddressed mail remain unread and retryable.
