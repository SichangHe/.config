...
Self-addressed manager-thread replies generated by `email_me.py --manager-human` always end in a final `PWD: NAME` footer, including when called with `--no-pwd-footer`, and are ignored instead of being surfaced back to the manager as inbound pending work. The watcher uses this final unquoted footer for manager-authored mail and continues to recognize already-sent `tmux` footers; quoted footer text and messages whose display name is `Human` remain eligible as human replies. Ignored self-authored echoes are also excluded from the manager-mail threshold counters so they do not create compression or cleanup pending work.
