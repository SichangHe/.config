...
Configure these together in `~/.config/omo_manager/local.env`:
...
export OMO_HUMAN_EMAIL_CONFIG_PATH="$HOME/.config/himalaya/config.toml"
```

`OMO_HUMAN_EMAIL_CONFIG_PATH` remains the separate Himalaya configuration for cleanup of the human mailbox. Cleanup accepts only `[a]` mail sent from the configured agent address to the configured human address.
