...
# 🧑 "a fresh independent ... scan must not escalate correctly indexed Human-held ... or helper-blocked ... as actionable ready agents, while genuine blocked-ready cases remain visible."
...
EXPECTED_STOPPED_BLOCKED_TASKS: dict[str, tuple[str, str, str, bool, str, str, str]] = {
...
def is_expected_intentionally_stopped_blocked_task(root: Path, task: TaskLine, state: TaskState, classified: StatusRow) -> bool:
    """Return whether exact stopped-record evidence makes an absent runtime expected."""

    task_path = resolve_task_path(root, task.task_file)
    indexed = [linked for linked in parse_task_lines(root / "TODO.md") if resolve_task_path(root, linked.task_file) == task_path]
    expected = EXPECTED_STOPPED_BLOCKED_TASKS.get(task.task_file)
    if (
        task_path is None
        or expected is None
        or state.status != "blocked"
...
def add_blocked_idle_vl_row(root: Path, task: TaskLine, role: str, rows: list[StatusRow], seen: set[str], manager_target: str = "", auto_unstick: bool = False, unstick_by_target: dict[str, str] | None = None, auto_unstick_disabled_reason: str = "not_problems_only") -> None:
    ...
    """
    state_path = resolve_task_path(root, task.task_file)
    if task_has_pending_marker(state_path) and task.task_file not in EXPECTED_STOPPED_BLOCKED_TASKS:
        return
    state = scan_task_state(state_path, root) if state_path is not None else None
    ...
    if target:
        ...
        if is_human_token_quota_pause(root, task, state) and idle_status in {"missing", "not_codex"}:
            return
        if is_expected_intentionally_stopped_blocked_task(root, task, state, classified):
            return
